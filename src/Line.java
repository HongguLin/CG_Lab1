			...
			public void run() {
				...
				Line line = new Line() {
					void line(BufferedImage buf, int x0, int y0, int xn, int yn, int rgb) {
						...
						/*
						//initialize width w and height h
						int w = xn-x0;
						int h = yn-y0;
						float x = x0;
						float y = y0;

						// the number of steps is initialize to the longest one between w and h
						int steps = Math.max(Math.abs(w), Math.abs(h));

						// the per step increment of x and y
						float xinc = (float)w/steps;
						float yinc = (float)h/steps;

						//set the start pixel point of the line
						buf.setRGB(Math.round(x),Math.round(y),rgb);
						// set each pixel of the line according to the increment
						...
						*/
						...
						//initialize all the per step increment to 0;
						int dx1=0,dy1=0,dx2=0,dy2=0;

						// dx1,dy1 are for the case that pk>0
						// if w>0 the increment should be positive, else should be negative, the same as h;
						if(w>0) dx1=1;if(w<0) dx1=-1;
						if(h>0) dy1=1;if(h<0) dy1=-1;

						// dx2, dy2 are for the case that pk<0, if w>h, set dy2=0, else,set dx2=0
						if(w>0) dx2=1;if(w<0) dx2=-1;

						// initialize longest to the abs largest one of w and h
						// initialize shortest to the abs smallest one of w and h
						...
						if(longest<=shortest){
							longest = Math.abs(h);
							shortest = Math.abs(w);
							if(h>0) dy2=1;if(h<0) dy2=-1;
							dx2=0;
						}
						// initialize p0, pk and d
						int d = 2 * (shortest-longest);
						int p0 = 2*shortest-longest;
						int pk = p0;

						for(int i=0;i<=longest;i++){
							buf.setRGB(x0,y0,rgb);
							// increase x0,y0 according to pk and reset the value of pk according to the previous pk
							if(pk<0){
								pk += 2*shortest;
								x0 += dx2;
								y0 += dy2;
