            ...
            public void run() {
                ...
                Line line = new Line() {
                    void line(BufferedImage buf, int x0, int y0, int xn, int yn, int rgb) {
                        ...
                        float gradient = (float) h/w;
                        float incy = y0 + gradient;
                        for(int i=x0+1;i<xn;i++){
                            int y = (int) Math.floor(incy);
                            int rate = (int) Math.floor((incy-(float) y)*100);
                            Color c1=new Color(rgb),c2,c3,mc2,mc3;
                            if(flag==0){
                                c2=new Color(buf.getRGB(i,y));
                                mc2=calculateColor(c1,c2,rate);
                                buf.setRGB(i,y,mc2.getRGB());

                                c3=new Color(buf.getRGB(i,y+1));
                                mc3=calculateColor(c1,c3,100-rate);
                                buf.setRGB(i,y+1,mc3.getRGB());
                            }else {
                                c2=new Color(buf.getRGB(y,i));
                                mc2=calculateColor(c1,c2,rate);
                                buf.setRGB(y,i,mc2.getRGB());

                                c3=new Color(buf.getRGB(y+1,i));
                                mc3=calculateColor(c1,c3,100-rate);
                                buf.setRGB(y+1,i,mc3.getRGB());
                            }
                            incy += gradient;
                        }
                    ...
                    protected Color calculateColor(Color c1, Color c2, float a){
                        if(a<0){
                            a=0;
                        }else if(a>100){
                            a=100;
                        ...
                        int R = (int) ((c1.getRed()*(100-a)+ c2.getRed()*a)/100);
                        int G = (int) ((c1.getGreen()*(100-a)+ c2.getGreen()*a)/100);
                        int B = (int) ((c1.getBlue()*(100-a)+ c2.getBlue()*a)/100);
